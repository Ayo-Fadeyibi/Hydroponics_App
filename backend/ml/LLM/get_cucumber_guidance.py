...
from Cucumber.predict import optimize
...
gen_config = types.GenerateContentConfig(
    system_instruction=(
        ...
        "practical, safe, and non-contradictory advice for growing Cucumbers. \n\n"
        ...
        "- Always mention the plant's name (Cucumber) in the summary to increase engagement."
    ),
    temperature=0.1, 
...
def sanitize_metrics(metrics_dict):
    """Helper function to convert numpy types to rounded python floats.
...
def get_cucumber_llm_guidance(current_metrics, locked_variables, optimal_environment=None):
    """
    Function to get LLM guidance on growth metrics on cucumbers.
    ...
    safety_config = {
    'Avg_Air_Temp':         {'dead': (20, 32)}, 
    'Avg_RH': {'dead': (50, 102)}, 
    'Avg_CO2':         {'dead': (300, 1050)}, 
    'Avg_EC':         {'dead': (0.5, 6)},
    'Avg_pH':   {'dead': (5.5, 8)}, 
    'Avg_DLI':         {'dead': (10, 65)}
    }
    ...
    # 3. Constructing the Data Payload
    data_payload = f"""
