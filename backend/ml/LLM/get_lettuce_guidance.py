...
from Lettuce.predict import get_safety_config, get_optimal_environment
...
class HydroponicAdvice(BaseModel):
    ...
    advice_summary: str = Field(description="2-sentence summary of the situation")
...
gen_config = types.GenerateContentConfig(
    system_instruction=(
        ...
        "practical, safe, and non-contradictory advice for growing lettuce. \n\n"
        ...
        "- UNIFIED NUTRIENTS: Group EC and TDS adjustments into a single step. Never suggest "
        "adding water and adding nutrients at the same time.\n\n"
        ...
        "- Always mention the plant's name (Lettuce) in the summary to increase engagement."
    ),
    temperature=0.1, 
...
def sanitize_metrics(metrics_dict):
    """Converts numpy types to rounded python floats.
    
     Parameters:
     ----------------------
     metrics_dict: dictionary of metrics to convert
     Returns:
     ----------------------
     A dictionary with the same keys and converted values.
    """
    return {
...
def get_lettuce_llm_guidance(current_metrics, locked_variables, optimal_environment=None):
    """
    Function to get LLM guidance on growth metrics on lettuce.
    ...
    if optimal_environment:
        ...
        raw_optimal = get_optimal_environment(locked_variables)
    ...
    - TDS: Current {clean_current.get('Avg_TDS')} (Target {clean_optimal.get('Avg_TDS')})
